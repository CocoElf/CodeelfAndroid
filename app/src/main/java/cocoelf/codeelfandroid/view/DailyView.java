package cocoelf.codeelfandroid.view;
...
import cocoelf.codeelfandroid.util.DateFactory;
import cocoelf.codeelfandroid.util.Day;
import cocoelf.codeelfandroid.util.UI;
...
public class DailyView extends View {
    ...
    public void initView() {
        mDays = DateFactory.getDays(2018, 1);
        //方格画笔
        boxPaint = new Paint();
...
//                layout(getLeft() + offsetX, getTop() + offsetY, getRight() + offsetX, getBottom() + offsetY);
     ...
     * @param hour 时间
     */
    public void setData(int year,int month,int day,int hour){
         //先找到是第几天，为了方便不做参数检测了
        for (Day d : mDays) {
            if (d.year == year && d.month == month && d.date == day){
                d.hour = hour;
                d.colour = getColour(hour);
                break;
            }
        }
        refreshView();
